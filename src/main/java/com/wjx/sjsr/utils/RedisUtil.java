...
public class RedisUtil {
    ...
    //切换数据库
    public void changeDatabase(Integer dbIndex){
        LettuceConnectionFactory lcf = (LettuceConnectionFactory) this.stringRedisTemplate.getConnectionFactory();
        if (lcf != null) {
            RedisStandaloneConfiguration sc = lcf.getStandaloneConfiguration();

            LettuceClientConfiguration lcc = lcf.getClientConfiguration();
            RedisStandaloneConfiguration rac = new RedisStandaloneConfiguration(sc.getHostName(),sc.getPort());
            rac.setPassword(sc.getPassword());
            rac.setDatabase(dbIndex);
            LettuceConnectionFactory lettuceConnectionFactory = new LettuceConnectionFactory(rac, lcc);
            lettuceConnectionFactory.afterPropertiesSet();
            this.stringRedisTemplate.setConnectionFactory(lettuceConnectionFactory);
        }
    }
    ...
    public boolean setLock(String lockKey, String lockName, long expireTime){
        return stringRedisTemplate.opsForValue()
                .setIfAbsent(lockName + ":" + lockKey, lockKey, expireTime, TimeUnit.SECONDS);
    }

    /**
     * 删除锁
     */
    public void delLock(String lockKey, String lockName){
        delete(lockName+":" + lockKey);
    }
     ...
     * @param key
     * */
    public void setLoginSms(String key, String value, Integer expire, Integer sendInterval){
        set("login_sms:" + key,value, expire);//缓存验证码
     ...
     * @param key
     * */
    public Object getLoginSms(String key){
        return stringRedisTemplate.opsForValue().get("login_sms:" + key);
    }
     ...
     * @param key 判断的key
     */
    public Boolean hasLoginSms(String key) {
        return stringRedisTemplate.opsForHash().getOperations().hasKey("login_sms:" + key);
    }
    /**
     * 删除验证码
     *
     ...
     * @return
     */
    public Long smsOneDaySendCount(String key){
        String count = get("phone_sms_count:" + key);
     ...
     * @return
     */
    public Boolean isJustSend(String phone){
        return hasKey("just_send_sms_phone:" + phone);
    }
